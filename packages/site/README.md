# Blind Escrow - FHEVM React Frontend

Blind Escrow là một dApp OTC trading sử dụng Fully Homomorphic Encryption (FHE) trên FHEVM.
Dự án này cung cấp giao diện frontend để tương tác với smart contract `BlindEscrow.sol`.

Dự án hỗ trợ chạy trên cả Sepolia testnet và local Hardhat Node.
...
- **`<root>/packages/site/hooks/useBlindEscrow.ts`**: React custom hook để tương tác với BlindEscrow contract, bao gồm tạo deal, submit giá, reveal và settle.
